...
% Bounds on kinetic parameters. Based mostly on Bar-Even paper (kcat 1/s, Km M)
ub = [1e+4,1e+4,1e-3,1e-3,Keq];
lb = [1e+0,1e+0,1e-6,1e-6,Keq];

% Define convex space (constraints)
Aeq = [1,-1,-1,1,-1];      % Haldane relationship
beq = 0;
% Aineq = [];
% bineq = [];
Aineq = [1,0,-1,0,0;...    % Diffusion limit
         0,1,0,-1,0];
bineq = log([1e9;1e9]);
...
%% Sampling
% Define parameters of the General Hit And Run Sampler
nSamples = 2e4;
nSteps   = 2e2;
nDiscard = 1e5;
